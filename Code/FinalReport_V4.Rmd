...
```{r model-sarimax, echo=F, warning=FALSE}
...
```{r model-mean, echo=F, warning=FALSE}
# Model 4
...
```{r model-nnetar, echo=F, warning=FALSE}
# Model 7: 
...
```{r model-arimax-price, echo=F, warning=FALSE}
# Model 8: 
...
**Interpretation.** SARIMAX tops the ranking at *3.1 %*, indicating that Block 43 output and WTI prices materially improve forecasts.  NNAR‑F follows closely (*3.4 %*), suggesting limited non‑linear gains once exogenous terms are included.  TBATS performs well but struggles with 2023 maintenance shocks.
...
```{r block43-average, echo=F, warning=FALSE}
...
#Model 1: ARIMA
```{r ARIMAannex, echo=F, warning=FALSE}
#Model 1: ARIMA
# Fit an ARIMA model to the annual time series and forecast for 3 years
...
```{r MEANannex, echo=F, warning=FALSE}
#Model 2: Arithmetic mean on original data
...
```{r ETSannex, echo=F, warning=FALSE}
# Model 3: ETS (Exponential Smoothing without seasonality)
...
```{r Holtannex, echo=F, warning=FALSE}
# Model 4: Holt's Linear Trend method
...
```{r accuracy2019, echo=F, warning=FALSE}
#Model 1: ARIMA
...
```{r scoresannex, echo=F, warning=FALSE}
#create data frame
...
```{r tableannex, echo=F, warning=FALSE}
kbl(models_scores_p, 
